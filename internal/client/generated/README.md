...
```go
import bitbucket "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `bitbucket.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `bitbucket.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `bitbucket.ContextOperationServerIndices` and `bitbucket.ContextOperationServerVariables` context maps.
